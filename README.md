...
这正是社区仅凭对局内观察、用统计归纳法就能摸出规律（[社区实测记录](社区实测记录.md) §2/§3 收录的 NZ33 代价表、环形游走与完整周期），乃至据此设计出"零费用控制天气结果"打法的根本原因。官方已于 2026-08-23 发布《关于秋季锦标赛预报机制的说明》，确认"尝试利用特定操作控制预报机制结果"属实，将其定性为游戏机制层面的问题，而非外挂/作弊；2026-08-25 官方发布[后续说明](https://www.kards.com/news/forecast-issue-and-fall-2026-tournament-results)，把成因归结为预报代码与部分旧版游戏代码之间一处未被测试覆盖到的意外交互，表示已经对系统做出调整使其更难被操纵、后续还有进一步改进计划，秋季锦标赛成绩维持不变、不追究任何参赛者责任。这次调整不需要更新客户端，说明它改动的是服务端下发/校验的参数或流程，而不是客户端里 §1 这套引擎自带的 LCG 算法本身——那部分是可以直接从 UE5.6 引擎源码读到的公开确定性算法，不受服务端更新影响。社区在调整生效后测试反馈：以前摸出来的这套操作规律（NZ33 代价表、环形游走等）已经不能再稳定复现期望结果。这说明**社区那套纯靠对局内观察归纳出的经验规律**已经失效；本文 §1-§6 依据引擎源码和常数推导出的重播种公式本身，尚未有人拿调整后的真实对局单独验证过是否还能按公式预测出结果——这是区别于"经验规律失效"的另一件事，具体见 §7 第 6 条。§1-§6 的静态推导和历史抓包证据本身没有被推翻，继续作为这套机制曾经可被利用的完整技术记录保留。
...
1. **`action_id` → 具体展示结果的精确映射公式**：数组下标↔小组编号的映射本身**已完全解决**（Weather.md §4.2）——`GetAllStaticCardsSortedByName` 的排序比较函数（`sub_141376EA0`）反编译确认是标准的、不区分大小写的 `FName` 字典序比较，`GetChooseSpawnCards` 的填桶逻辑确认每个磁级的桶只装同类型同磁级的 3 个小组变体，三者资产名恰好是"无后缀/加2/加3"，字典序天然给出下标 0/1/2 = 小组 1/2/3，不需要任何运行时数据。进一步用已确认的 LCG 常数精确计算离散步长分布（不只是连续漂移量的最小值）、并排查了"前端交互时序改变变换深度"这个候选解释（反编译 `HandleSelectCardToDrawPending`→`AddSubActionSelectCardToDrawPending` 调用链确认弹窗通知是一次性的、非交互式的，排除了这个解释）后，社区数据中"2K/4K 代价表完全相同"（[社区实测记录](社区实测记录.md) §2.1）跟已确认引擎机制之间的关系已经精确到"哪一项吻合、哪一项不吻合"：`d=15`（"同小组"）在 light、medium 两档上都能算出"步长以高概率为 0"，跟经验数据吻合；`d=3`（"顺时针跨组"）在两档上算出的主导步长方向相反，跟经验数据矛盾——完整推导见 Weather.md §4.2。剩下的具体矛盾（"顺时针跨组"这一项）尚未定位到成因，候选来源包括 NZ33 数据本身的精确度（本报告没有渠道独立核实）和"跨天气类型触发预报"这类尚未验证的场景。
...
6. **2026-08-25 服务端调整后，§1-§6 的重播种公式本身是否还能预测出结果**：官方说明只确认"已经做出调整使其更难被操纵"，没有给出技术细节，服务端代码也不在本报告的静态分析范围内。目前能确认的只是社区那套纯靠对局内观察归纳出的经验规律（NZ33 代价表、环形游走等）在调整后已经测不出来了；但客户端引擎自带的 LCG 算法本身没有理由被这次调整改变（调整不需要更新客户端），本文 §1-§6 的公式是否依然能算出服务端最终确认的结果，还没有人拿调整后的真实对局单独测过。更可能打断"经验规律"的候选是服务端改变了权威结果的产生方式（比如权威结果不再单纯由 `CurrentActionId` 决定）、或者服务端增加了针对高频操纵序列的校验/丢弃逻辑——区分这几种可能，以及验证公式本身是否依然成立，都需要调整后的新真实抓包，目前没有这类数据。
