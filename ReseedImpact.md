...
`bUseTurnSwitchValidation` 是服务端随每场对局引导数据下发的值（README.md §2.2 已用 `xrefs_to_field` 排除了客户端硬编码赋值的可能），不是编译进客户端二进制的常量，所以它在 PvP/天梯/锦标赛对局下具体取什么值，客观上不属于"反编译这份客户端二进制能回答的问题"——需要那些对局类型各自的真实抓包才能拿到答案，这是数据覆盖范围的边界，不是机制本身存在疑点。本报告目前掌握的抓包只有 `match_type: "training"`（人机单机局）两份，均确认该值为 `true`；据此，本报告的全部结论（§1-3）明确限定在 `training` 对局范围内。若要把结论扩展到 PvP/天梯/锦标赛，唯一途径是补充那些对局类型的真实抓包。
