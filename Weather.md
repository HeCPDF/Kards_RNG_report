...
**把这个已确定的映射代入完整模型后，可以对每个 tier 精确计算离散步长分布**：跟 SpyRing.md §3 同样的方法（把 `Δ_seed(d) = d×19390×A^k mod 2^32` 代入离散化规则，`A^k` 换成对应 tier 的变换深度），逐个整数 `d`（本地计数器步进量）测出每个 tier 在每个 `d` 下的**离散下标步长分布**（不只是连续漂移量的最小值，而是"哪个具体步长值占主导、概率多少"），20000 次采样：
...
**`d=15` 时，light 和 medium 的主导步长都是 `0`（同小组、结果不变）**——这跟 NZ33 报告的"2K、4K 同小组代价都是 15"直接吻合，且是两个 tier 同时成立，不是像之前只看"连续漂移量最小值"那样只对 light 成立。这是一个比早期版本更精确的方法：早期版本只找"连续漂移量最小的 `d`"（medium 的连续漂移最小点确实在 `d=3`，不在 `d=15`），但"连续漂移量最小"和"离散步长以多高概率保持不变"是两个不同的问题——`d=15` 虽然不是 medium 连续漂移的全局最小点，但离散化之后仍然是"步长 0"占主导（61.3%），跟 light 在同一个 `d` 上给出同样的主导结论。

但 `d=3`（NZ33 报告的"顺时针跨组代价"）在两个 tier 上**仍然对不上**：light 在 `d=3` 的主导步长是 `-1`（跨了一组，59.0%），medium 在 `d=3` 的主导步长却是 `0`（没跨组，92.0%，比 light 的"跨组"结论更确定）——这跟"2K/4K 顺时针跨组代价都是 3"直接矛盾，且这次矛盾的方向更明确：不是"数字对不上"，而是"medium 在 `d=3` 时压根不太可能跨组"。`d=15` 一项的吻合，加上 `d=3` 一项的持续矛盾，共同说明：驱动展示结果的确定性机制（重播种公式 + 三次固定深度变换）已经确认无误，NZ33 代价表里"同小组"这一项数字現在有了扎实的引擎层解释，但"跨组"这一项数字仍未被解释——这把此前"整张表都对不上"的笼统矛盾，进一步收窄成了"具体是哪一项对不上"，是更精确的待查状态。
...
排除了"重播种发生多次"（已排除）、"数组下标映射错误"（已确定是简单字典序，不是错误来源）、"前端交互时序影响变换深度"（已排除）这三个可能的技术性解释之后，NZ33"2K 与 4K 代价表完全相同"这条经验结论，现在跟本报告已确认的引擎机制之间的关系是**部分吻合、部分仍不吻合，且两部分都已精确定位**：`d=15`（"同小组"这一项）在 light、medium 两档上都能算出"步长以高概率为 0"，跟经验数据吻合；`d=3`（"顺时针跨组"这一项）在两档上算出的主导步长方向相反（light 倾向跨组、medium 倾向不跨组），跟"两档代价相同"这一条经验描述矛盾。剩下的差距不再是"整张表都对不上、原因不明"，而是聚焦到"顺时针跨组"这一项具体数字上——可能来源包括：NZ33 代价表这一项本身的精确度（本报告没有独立核实过其原始测试数据）、或者玩家在两次预报之间实际选择的天气"类型"是否相同这类目前没有建模进来的变量（跨类型触发时，`GetChooseSpawnCards` 走的是不同天气类型代表卡各自独立的 `_lightWeatherCards` 等数组，但用的是同一条 `cardsRandomStream`，尚未验证跨类型场景下的具体表现是否跟本节的单类型推导一致）。

结论：驱动 2K/4K/6K 展示结果的具体机制，不是"三次连续调用的原始返回值按固定深度 1/2/3 直接展示"，这一点现在有三重证据（抽象仿射证明、具体数值代入、以及排除"前端时序"这个候选解释后仍然矛盾）；数组下标↔小组编号的映射关系本身已经确定（字典序，不需要运行时验证）；light=2K/medium=4K/heavy=6K 的档位对应关系已经用 `kredits` 字段核实无误；NZ33 代价表里"同小组"这一项数字已经用引擎常数精确复现，"顺时针跨组"这一项仍然对不上，是目前唯一具体、已精确定位、但成因未知的残留矛盾。
