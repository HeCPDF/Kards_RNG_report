...
| 函数 | 位置 | 作用 |
|---|---|---|
| `RandomIntFromRangeWithStream(min, max, out result)` | `BP_CardFunctions.cpp:12423` | 唯一的随机数入口封装，内部就是一行 `UKismetMathLibrary::RandomIntegerInRangeFromStream(cardsRandomStream, min, max)`。全游戏所有卡牌（天气、间谍组织、任何"随机抽一个"效果）都通过这个函数取随机数，没有第二条路径。 |
...
天气系统的卡牌全部放在 `Content/Blueprints/Cards/Neutral/events/` 下，命名规律是 `card_event_<天气类型><级别>_<卡名>`（`sunny1`=一级晴天代表卡，`sunny2_heatwave`=晴天的二级效果卡……以此类推，`rain`/`storm` 同理）。这些卡全部属于"活跃卡池"（Active Static Cards），运行时全程被加载——不是打出来才实例化，而是每一局开始就作为场上常驻的静态对象存在，随时可被 `GetAllActiveStaticCards()` 枚举到。
...
`sunny1_blue_sky` 是一级代表卡，本身不带 `subtype.*` 分级标签（它只是 UI 上给玩家选的三个选项之一），但带着 `autoplay` 标签——这张卡一旦加入手牌，游戏会自动打出它，不需要玩家再手动点一次"出牌"。这解释了预报为什么只需要玩家做两次选择（天气类型、天气强度）而不是三次：玩家在第一个弹窗里选中某个天气类型后，对应的一级代表卡被加入手牌并因 `autoplay` 立刻自动打出，触发 `OnPlayedFromHand`，紧接着弹出第二个窗口（轻/中/重三选一）。真正携带分级信息的是各个效果卡，例如同目录下的 `card_event_sunny2_heatwave`：
...
`subtype.sunny`/`subtype.rain`/`subtype.storm` 标记天气种类，`subtype.lightWeather`/`subtype.mediumWeather`/`subtype.heavyWeather` 标记轻/中/重（俗称 2K/4K/6K）级别。天气系统靠遍历全部活跃卡、按这两组标签分类，不是靠一张写死的表——新增一张天气卡，只要打上正确的标签、丢进 `Neutral/events/` 目录，就会自动被下面这套逻辑收编。
...
`cardFunction->RandomIntFromRangeWithStream(min, max, out)`（`BP_CardFunctions.cpp:12423`，蓝图，一行转发）调用 `UKismetMathLibrary::RandomIntegerInRangeFromStream`。这个原生函数在 `kards-Win64-Shipping.exe` 里对应 IDA 地址 `0x143ddce50`，反编译如下（`v7` 指向 `FRandomStream` 结构体，`+0` 是 `InitialSeed`，`+4` 是 `Seed`；`v11`=`min`，`v12`=`max-min+1`）：
...
重播种：`seed = 1000000000 + 10*19390 = 1000193900`。

`GetChooseSpawnCards` 依次调用三次 `RandomIntFromRangeWithStream(0, 2)`：

| 第几次调用 | 变换前 Seed | 变换后 Seed | 返回下标 |
|---|---|---|---|
