...
原本 ReseedImpact.md 暗示的"`action_id` 是小整数、容易脱离对局历史单独预测"的说法需要修正：`CurrentActionId` 的取值依赖于——已处理的动作序列里每一条的**类型**（是否 `XActionStartOfTurn`/`XActionEndOfTurn`/`XActionMulligan`）、**归属方**、是否重复、以及本地动作日志哈希表当前的状态，还会在特定条件下被整体重置为 `1`。要在不实际重放这整套状态机的前提下,单纯"数一数这是第几个动作"来推算 `CurrentActionId`,是不可靠的——这跟直接解析完整的动作日志(抓包+脚本解析,如用户所说)在工作量上没有本质区别,不是一条更简单的捷径。ReseedImpact.md 已同步修正这一点。
