# 真实对局实时验证：mitmproxy 被动抓包，8 次预报全部命中已确认公式
...
第二场（`match_id=[已脱敏]`，7/8 为同一回合内背靠背连续触发、中间无其他操作）：
...
**没有出现任何一次矛盾**——每一次玩家实际看到的展示结果，都能在已确认公式算出的候选集里找到，没有一次是"任何 `action_id` 取值都算不出这个组合"的情况。三元组 `(light,medium,heavy)` 命中概率若纯属巧合，单次约为 `1/27`，八次全部不矛盾在零假设下概率极低——这是对 README.md §1/§2、Weather.md §4.2 核心公式最强的一次实证。
...
## 副产品发现 4：一次真正"背靠背"的连续两次预报，实测 `CurrentActionId` 相差 8 或 11——远大于此前假设的 3

在第二局真实对局（PvP 休闲模式，`match_id=[已脱敏]`，好友对战）里，玩家连续触发了两次预报，中间没有做任何其他操作（第二次预报紧接着第一次的结果弹窗出现）：
...
**这意味着"背靠背连续两次预报，中间不做任何其他操作"，`CurrentActionId` 实测相差 8 或 11 个单位**——远大于 ReseedImpact.md §1 原本从动作创建链反编译推出的"3 个单位"，也印证了副产品发现 3 的结论：这个推导需要重新核实，真实的单位消耗数明显更大、更复杂，可能包含没有走 `POST .../actions` 网络提交、但仍然会被 `GetNextAction_Impl` 处理进 `CurrentActionId` 推进逻辑的本地/隐式事件。
